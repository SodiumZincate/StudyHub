...
try { window.location.href = '/api/discussion/answer'}
...
ask.addEventListener("click", (event) => {
  event.preventDefault();
  try { window.location.href = '/api/discussion/ask' }
  catch(error) { console.error("There was an error with the request:", error) }
})
...
form.addEventListener('submit', function (event) {
	...
	axios.post('/api/discussion/add-question', data)
		.then(response => {
...
const block1 = async function () {
	...
	buttons.forEach(function (button) {
		button.addEventListener('click', function (event) {
			...
			axios.get(`/api/discussion/get-a-for-q?questionID=${questionID}`)
			.then((res) => {
...
const block2 = function () {
	...
	deleteButton.addEventListener("click", (event) => {
		...
		axios.delete(`/api/discussion/delete-question?questionID=${questionID}`)
			.then((res) => {
...
axios.get(`/api/discussion/get-your-questions/?page=${page}`)
.then((res) => {
...
document.querySelector('.main-section-1').addEventListener('submit', async function (event) {
    ...
    if (query.trim()) {
		await axios.get('/api/discussion/search', {
			params: { 
				item: query, 
